...
namespace Joomla\Plugin\System\wt_index_now\Extension;
...
final class wt_index_now extends CMSPlugin implements SubscriberInterface
{
	...
	{
		return [
			'onwt_index_nowSendMessage' => 'onwt_index_nowSendMessage'
		];
	}

	public function onwt_index_nowSendMessage(Event $event): void
	{
...
//				$telegram_method   = 'sendMediaGroup';
	...
	{
		\JLoader::registerNamespace('WPSocio\TelegramFormatText', JPATH_SITE.'/plugins/system/wt_index_now/src/Lib/telegramformattext/src');
	...
	private function saveTelegramMessageId($telegram_response, array $message_params = []): void
	{
		...
		$query->insert($db->quoteName('#__plg_system_wt_index_now'));
