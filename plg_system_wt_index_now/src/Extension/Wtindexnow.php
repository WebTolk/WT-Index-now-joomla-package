...
final class Wtindexnow extends CMSPlugin implements SubscriberInterface
{
	...
	{
		return [
			'onWtindexnowSendMessage' => 'onWtindexnowSendMessage'
		];
	}

	public function onWtindexnowSendMessage(Event $event): void
	{
...
//				$telegram_method   = 'sendMediaGroup';
	...
	{
		\JLoader::registerNamespace('WPSocio\TelegramFormatText', JPATH_SITE.'/plugins/system/wtindexnow/src/Lib/telegramformattext/src');
	...
	private function saveTelegramMessageId($telegram_response, array $message_params = []): void
	{
		...
		$query->insert($db->quoteName('#__plg_system_wtindexnow'));
