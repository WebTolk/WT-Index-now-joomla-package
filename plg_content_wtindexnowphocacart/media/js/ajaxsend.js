 ...
 */
(() => {
	window.wtindexnowphocacart = () => {

		let item_ids = [];

		let currentUrl = new URL(window.location.href);
		let view = currentUrl.searchParams.get('view');
		if ((view === 'phocacartitem' || view === 'phocacartcategory') && currentUrl.searchParams.get('layout') === 'edit') {
		...
		let context =  (view === 'phocacartcategories' || view === 'phocacartcategory' ) ? 'com_phocacart.phocacartcategory' : 'com_phocacart.phocacartitem';

		Joomla.request({
			url: 'index.php?option=com_ajax&plugin=wtindexnowphocacart&group=content&format=json&' + Joomla.getOptions('csrf.token') + '=1',
			method: 'POST',
			data: JSON.stringify({
				'item_ids': item_ids,
				'context': context,
